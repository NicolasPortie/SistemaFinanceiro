        ...
        new
        {
            ...
            function = new
            {
                ...
                parameters = new
                {
                    ...
                    properties = new
                    {
                        comandoInterno = new { type = "string", description = "O comando que ele quer executar. Ex: listar_faturas, ver_fatura, ver_categorias, consultar_metas, consultar_limites, ver_extrato, comparar_meses, excluir_lancamento, ver_score, ver_perfil, ver_sazonalidade, ver_recorrentes, ver_lembretes, ver_salario, criar_categoria. Se for uma pergunta fora desses fluxos, use 'none' (resposta livre)." },
                        parametro = new { type = "string", description = "Qualquer entidade textual que precise acompanhar o comando interno (ex: o nome do cartão a ser excluido, a categoria detalhada, a descrição do lancamento excluido). Para excluir_lancamento: use a descrição do lançamento mencionada pelo usuário. Se o usuário disser 'excluir último' ou 'apagar último lançamento', envie 'ultimo' como parametro. Se não especificou qual, envie vazio. Para criar_categoria: envie SOMENTE o nome da nova categoria (ex: 'Roupas', 'Pet', 'Academia') — nunca uma frase completa. Para comparar_meses: envie 'mes_atual_vs_mes_passado', 'mes_atual_vs_mes_retrasado' ou 'MM/AAAA_vs_MM/AAAA'. Para ver_extrato: envie o período solicitado no formato 'MM/AAAA' (mês inteiro) ou 'DD/MM/AAAA_DD/MM/AAAA' (intervalo específico). Ex: '02/2026' para fevereiro, '01/02/2026_20/02/2026' para intervalo. Se o usuário disser apenas o nome do mês, deduza o ano pela Data Atual. Para detalhar_categoria: envie o nome da categoria. Se o usuário especificar um período junto, use o formato 'NomeCategoria|MM/AAAA' ou 'NomeCategoria|DD/MM/AAAA_DD/MM/AAAA'. Ex: 'Alimentação|02/2026', 'Transporte|01/03/2026_31/03/2026'." },
