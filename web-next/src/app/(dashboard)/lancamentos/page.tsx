          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={handleRefresh} className="p-2.5 hover:bg-white/60 dark:hover:bg-slate-700/60 rounded-xl transition-colors cursor-pointer">
          ...
          >
            <div className="flex items-center justify-between p-3 px-5 rounded-2xl bg-emerald-600/10 border border-emerald-600/20 text-emerald-600 dark:text-emerald-400">
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => setSelectedIds([])} className="hover:bg-emerald-600/10">
                  Cancelar
                </Button>
                <Button variant="destructive" size="sm" onClick={() => setDeletingMany(true)} className="gap-2 shadow-sm rounded-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-32 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
            ...
            <div className="z-10 mt-auto">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{formatCurrency(resumo.totalReceitas)}</h3>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-32 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
            ...
            <div className="z-10 mt-auto">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{formatCurrency(resumo.totalGastos)}</h3>
            </div>
          </motion.div>

          {/* Saldo */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-32 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300 ring-2 ring-emerald-600/20">
            <div className="absolute -right-6 -bottom-6 bg-emerald-500/10 w-28 h-28 rounded-full blur-2xl group-hover:bg-emerald-500/15 transition-all" />
            <div className="flex justify-between items-start z-10">
              <div className="size-10 flex items-center justify-center bg-emerald-100 dark:bg-emerald-500/15 rounded-xl text-emerald-600 dark:text-emerald-400">
            ...
            <div className="z-10 mt-auto">
              <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Saldo</p>
              <h3 className={cn("text-2xl font-bold tracking-tight", resumo.saldo >= 0 ? "text-slate-800 dark:text-white" : "text-red-600 dark:text-red-400")}>{formatCurrency(resumo.saldo)}</h3>
            </div>
          </motion.div>
        </div>
      ) : null}
      ...
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          ...
          <div className="relative flex-1 w-full lg:max-w-sm">
            ...
            />
            {busca && (
              <button onClick={() => { setBusca(""); setPagina(1); }} className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 rounded-md text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors cursor-pointer" aria-label="Limpar busca">
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {l.parcelado && (
                            <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium">{l.numeroParcelas}x de {formatCurrency(l.valor / l.numeroParcelas)}</p>
                          )}
                        </div>
                      </div>

                      {/* Date */}
                      <span className="text-[13px] text-slate-500 dark:text-slate-400 font-medium tabular-nums">{formatDate(l.data)}</span>
                      ...
                      <div>
                        <span className={cn(
                          ...
                          <span className={cn("h-1.5 w-1.5 rounded-full", getCategoryColor(l.categoria))} />
                          {l.categoria}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center justify-center h-7 w-7 rounded-lg bg-slate-100 dark:bg-slate-700/50">
                          <PaymentIcon method={l.formaPagamento} className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" />
                        </div>
                        <div className="min-w-0">
                          <span className="text-[12px] text-slate-600 dark:text-slate-300 font-medium">{formatFormaPagamento(l.formaPagamento)}</span>
                          {l.parcelado && (
                            <span className="ml-1.5 text-[10px] bg-slate-100 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded-md font-medium">{l.numeroParcelas}x</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => setDeletingId(l.id)} className="gap-2 text-red-600 dark:text-red-400 cursor-pointer">
                    ...
                    <div
                      className={cn(
                        ...
                        selectedIds.includes(l.id) ? "bg-emerald-600/5" : "hover:bg-white/30 dark:hover:bg-slate-800/20"
                      )}
                      onClick={() => setViewingItem(l)}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 w-full min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[12px] text-slate-400 dark:text-slate-500 font-medium truncate mt-0.5">{l.categoria}</p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          ...
                          <span className="block text-[11px] font-semibold text-slate-400 dark:text-slate-500 mt-1">{formatDate(l.data)}</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
          ...
          <SheetHeader className="px-5 sm:px-7 pt-5 sm:pt-6 pb-4 sm:pb-5">
            <div className={cn(
              ...
              <div className={cn(
                ...
                tipoSelecionado === "receita" ? "bg-emerald-500/15 text-emerald-600" : "bg-red-500/15 text-red-600"
              )}>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="px-5 sm:px-7 pb-8 space-y-4 sm:space-y-5">
              ...
              <div className="relative flex p-1 rounded-xl bg-muted/40">
                <div className={cn(
                  ...
                  tipoSelecionado === "receita" ? "left-1 bg-emerald-500 shadow-lg shadow-emerald-500/25" : "left-[calc(50%+3px)] bg-red-500 shadow-lg shadow-red-500/25"
                )} />
                <button
                  type="button"
                  ...
                  className={cn(
                    ...
                    tipoSelecionado === "receita" ? "text-white" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    "relative z-10 flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-colors duration-300 cursor-pointer",
                    tipoSelecionado === "despesa" ? "text-white" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <div className={cn(
                      ...
                      tipoSelecionado === "receita" ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"
                    )}>R$</div>
                    <CurrencyInput
              ...
              {tipoSelecionado === "despesa" && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    >
                      <div className={cn("flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-lg sm:rounded-xl transition-all", formaPagamentoSelecionada === "pix" ? "bg-emerald-600/10" : "bg-muted/40 group-hover:bg-muted/60")}>
                    ...
                    >
                      <div className={cn("flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-lg sm:rounded-xl transition-all", formaPagamentoSelecionada === "debito" ? "bg-emerald-600/10" : "bg-muted/40 group-hover:bg-muted/60")}>
                    ...
                    <button
                      type="button"
                      onClick={() => { form.setValue("formaPagamento", "credito"); form.setValue("contaId", ""); }}
                      className={cn(
                        "group relative flex flex-col items-center gap-1.5 sm:gap-2.5 py-3 sm:py-4 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-semibold transition-all duration-200 cursor-pointer border",
                    ...
                    >
                      <div className={cn("flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-lg sm:rounded-xl transition-all", formaPagamentoSelecionada === "credito" ? "bg-emerald-600/10" : "bg-muted/40 group-hover:bg-muted/60")}>
              ...
              {tipoSelecionado === "despesa" && formaPagamentoSelecionada !== "" && formaPagamentoSelecionada !== "credito" && (
                <div className="space-y-4 rounded-2xl border border-emerald-600/8 dark:border-slate-700/40 bg-white dark:bg-slate-800/60 shadow-[0_1px_6px_rgba(16,185,129,0.06)] dark:shadow-none p-5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Conta Bancária</span>
                  </div>
                  {contasBancarias.length === 0 ? (
                    <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
                      <p className="text-sm text-amber-600 dark:text-amber-400 font-medium">Nenhuma conta cadastrada.</p>
                      <p className="text-xs text-muted-foreground mt-0.5">Cadastre uma conta na página <span className="font-semibold">Contas</span>.</p>
                    </div>
                  ) : (
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium text-foreground/70">Conta <span className="text-muted-foreground font-normal">(opcional)</span></Label>
                      <Select value={contaSelecionada ?? ""} onValueChange={(v) => form.setValue("contaId", v === "__none" ? "" : v)}>
                        <SelectTrigger className="h-11 rounded-xl border-border/40 bg-background"><SelectValue placeholder="Selecionar conta" /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__none">Nenhuma</SelectItem>
                          {contasBancarias.map((c) => (<SelectItem key={c.id} value={c.id.toString()}>{c.nome}</SelectItem>))}
                        </SelectContent>
                      </Select>
                    </div>
                  )}
      ...
      <Dialog open={editingItem !== null} onOpenChange={() => setEditingItem(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={editForm.handleSubmit(onEdit)} className="space-y-5">
            {editingItem && (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/30 border border-border/30">
                <div className={cn(
                  ...
                  editingItem.tipo === "receita" ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400" : "bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400"
                )}>
            ...
            <Button type="submit" className="w-full h-11 rounded-xl gap-2 font-bold bg-emerald-600 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20" loading={atualizarLancamento.isPending}>
              Salvar alterações
            </Button>
          </form>
        </DialogContent>
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4 rounded-xl bg-muted/20 border border-border/30">
                <div className={cn(
                  ...
                  viewingItem.tipo === "receita" ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400" : "bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400"
                )}>
                ...
                <span className={cn(
                  ...
                  viewingItem.tipo === "receita" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
                )}>
