    ...
    <div className="relative w-36 h-36 xl:w-40 xl:h-40 flex items-center justify-center shrink-0">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
        ...
        <circle
          ...
          stroke="#10B981"
          strokeWidth="8"
          strokeDasharray={`${pctAtivos} ${251.2 - pctAtivos}`}
          strokeLinecap="round"
        />
        <circle
          cx="50"
          ...
          stroke="#6366f1"
          strokeWidth="8"
          strokeDasharray={`${pctInativos} ${251.2 - pctInativos}`}
          strokeLinecap="round"
          strokeDashoffset={`${251.2 - pctAtivos}`}
        />
      </svg>
      <div className="absolute text-center">
        <p className="text-xl serif-italic text-slate-900">
...
function ActivityBars({ data }: { data: { data: string; quantidade: number }[] }) {
  ...
  return (
    <div className="flex-1 flex items-end justify-between px-2 gap-3">
        ...
        return (
          <div key={d.data} className="flex flex-col items-center gap-2 flex-1 h-full justify-end">
            <div className="flex items-end gap-1 w-full justify-center h-[70%]">
              <div
                className={`w-3 rounded-t-sm ${isPeak ? "bg-emerald-500 shadow-[0_5px_15px_rgba(16,185,129,0.3)]" : "bg-emerald-500/40"}`}
                style={{ height: `${pct}%` }}
              />
            </div>
            <span
              className={`text-[9px] mono-data uppercase ${isPeak ? "text-emerald-600 font-bold" : "text-slate-400"}`}
            >
              {label}
            </span>
          </div>
      ...
      {last7.length === 0 && (
        <p className="text-xs text-slate-400 mx-auto self-center">Sem dados</p>
      )}
    </div>
  );
}
...
function PlanBadge({ role }: { role: string }) {
  if (role === "admin")
    return (
      <span className="px-3 py-1 bg-slate-900 text-white text-[10px] font-bold rounded-full uppercase tracking-wider">
        Admin
      </span>
    );
  if (role === "premium" || role === "executive")
    return (
      <span className="px-3 py-1 bg-slate-900 text-white text-[10px] font-bold rounded-full uppercase tracking-wider">
        Executive
      </span>
  ...
  if (role === "pro")
    return (
      <span className="px-3 py-1 bg-slate-200 text-slate-700 text-[10px] font-bold rounded-full uppercase tracking-wider">
        Pro
      </span>
    );
  return (
...
export default function AdminDashboardPage() {
  ...
  const pctAtivos = data.totalUsuarios
    ? Math.round((data.usuariosAtivos / data.totalUsuarios) * 100)
    : 0;
  const pctTelegram = data.totalUsuarios
    ? Math.round((data.usuariosComTelegram / data.totalUsuarios) * 100)
    : 0;
          ...
          {activeTab === "visao-geral" ? (
            <>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">
                MÃ©tricas de Crescimento e OperaÃ§Ã£o
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">
                VisÃ£o Geral e Controle de Acesso
              </p>
            </>
          )}
        </div>
          ...
          <div className="flex items-center bg-slate-100/80 p-1 rounded-full border border-slate-200/50">
            {tabs.map((t) => (
              <button
                ...
                className={cn(
                  "px-5 py-2 text-[9px] font-bold uppercase tracking-widest rounded-full transition-all",
                  activeTab === t.key
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8">
            <div className="exec-card rounded-2xl sm:rounded-[2.5rem] lg:rounded-[3rem] p-5 sm:p-8 lg:p-10 relative overflow-hidden group">
              ...
              <span className="text-2xl sm:text-3xl lg:text-4xl serif-italic text-slate-900 tracking-tight block mb-2">
                <AnimatedNumber value={data.totalUsuarios} />
              </span>
              <div className="flex items-center gap-2 text-[10px] mono-data text-emerald-600 font-bold">
                <TrendingUp className="w-4 h-4" />
                <span>+{data.novosUltimos7Dias} nos últimos 7d</span>
              </div>
            </div>
            <div className="exec-card rounded-2xl sm:rounded-[2.5rem] lg:rounded-[3rem] p-5 sm:p-8 lg:p-10 relative overflow-hidden group">
              ...
              <span className="text-2xl sm:text-3xl lg:text-4xl serif-italic text-slate-900 tracking-tight block mb-2">
                <AnimatedNumber value={data.totalLancamentosMes} />
              </span>
              <p className="text-[10px] mono-data text-slate-500 font-bold">
                {data.metasAtivas} metas ativas · {data.totalCartoes} cartões
              </p>
            </div>
            <div className="exec-card rounded-2xl sm:rounded-[2.5rem] lg:rounded-[3rem] p-5 sm:p-8 lg:p-10 relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity">
              ...
              <span className="text-2xl sm:text-3xl lg:text-4xl serif-italic text-slate-900 tracking-tight block mb-2">
                <AnimatedNumber value={data.sessoesAtivas} />
              </span>
              <p className="text-[10px] mono-data text-slate-500 font-bold">
                {data.usuariosComTelegram} vinculados ao Telegram
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
            <div className="exec-card rounded-2xl sm:rounded-[2.5rem] lg:rounded-[3rem] p-6 sm:p-10 lg:p-12 flex flex-col sm:flex-row items-center gap-6 sm:gap-10 xl:gap-16">
              <div className="flex-1 space-y-5">
                <h4 className="text-[9px] font-bold text-slate-900 uppercase tracking-[0.3em] mb-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {[
                    ...
                    {
                      label: "Inativos",
                      pct: data.totalUsuarios
                        ? Math.round((data.usuariosInativos / data.totalUsuarios) * 100)
                        : 0,
                      color: "bg-indigo-500",
                    },
                    {
                      label: "Bloqueados",
                      pct: data.totalUsuarios
                        ? Math.round((data.usuariosBloqueados / data.totalUsuarios) * 100)
                        : 0,
                      color: "bg-rose-400",
                    },
                  ].map((item) => (
                    ...
                    >
                      ...
                      <span className="text-slate-900 mono-data font-bold whitespace-nowrap">
                        {item.pct}%
                      </span>
                    </div>
                  ))}
              ...
              <DonutChart
                ativos={data.usuariosAtivos}
                inativos={data.usuariosInativos}
                bloqueados={data.usuariosBloqueados}
              />
            </div>
                    ...
                    <thead>
                      <tr>
                        {["CÃ³digo", "DescriÃ§Ã£o", "Uso", "Status", ""].map((h, i) => (
                          <th
                            key={i}
                            className="px-6 py-4 text-[8px] text-slate-400 font-bold uppercase tracking-widest border-b border-slate-100"
                          >
                            {h}
                          </th>
                        ))}
                      ...
                      {convites.map((c) => {
                        ...
                        return (
                          <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-6 py-5 mono-data text-[11px] font-bold text-slate-900">
                              {c.codigo}
                            </td>
                            <td className="px-6 py-5 text-[10px] text-slate-600 font-medium max-w-40 truncate">
                              {c.descricao ?? "â€”"}
                            </td>
                            <td className="px-6 py-5 text-[11px] mono-data text-slate-500">
                              {c.ilimitado ? "âˆž" : `${c.usosRealizados}/${c.usoMaximo ?? 1}`}
                            </td>
                            <td className="px-6 py-5">
                              {ativo ? (
                                <span className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[8px] font-bold uppercase tracking-widest">
                                  Ativo
                                </span>
                              ) : esgotado ? (
                                <span className="px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-[8px] font-bold uppercase tracking-widest">
                                  Esgotado
                                </span>
                              ) : c.expirado ? (
                                <span className="px-3 py-1 bg-amber-50 text-amber-600 rounded-full text-[8px] font-bold uppercase tracking-widest">
                                  Expirado
                                </span>
                              ) : (
                                <span className="px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-[8px] font-bold uppercase tracking-widest">
                            ...
                            <td className="px-6 py-5 text-right">
                              <button
                                onClick={() => deleteConviteMutation.mutate(c.id)}
              ...
              <div className="flex-1 bg-white px-8 py-6">
                {loadingUsers ? (
                  ...
                  <div className="flex items-center justify-center py-12">
                    <p className="text-xs text-slate-400">Sem cadastros recentes</p>
                  </div>
                ) : (
                ...
                {usuarios.length > 6 && (
                  <button
                    ...
                    className="mt-6 text-[9px] font-bold text-emerald-600 uppercase tracking-widest hover:text-emerald-700 transition-colors"
                  >
                    Ver todos â†’
                  </button>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="exec-card rounded-3xl p-6 flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-baseline gap-3">
                  <span className="text-3xl mono-data text-slate-900 font-bold">
                    <AnimatedNumber value={data.totalUsuarios} />
                  </span>
                  <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full">
                    +{data.novosUltimos30Dias} m/m
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="exec-card rounded-3xl p-6 flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-baseline gap-3">
                  <span className="text-3xl mono-data text-slate-900 font-bold">
                    <AnimatedNumber value={data.usuariosBloqueados + data.usuariosInativos} />
                  </span>
                  <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full">
                    {pctTelegram}% Telegram
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="exec-card rounded-[2rem] flex flex-col overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 bg-white flex flex-wrap items-center justify-between gap-4">
              <div className="relative w-full max-w-md">
                ...
                <input
                  className="w-full bg-slate-50 border border-slate-100 rounded-full pl-12 pr-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all placeholder:text-slate-400 text-slate-900"
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  className="bg-slate-50 border border-slate-100 rounded-full text-xs py-2 pl-4 pr-8 focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 font-medium cursor-pointer"
                  value={statusFilter}
                      ...
                      <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 border-b border-slate-50 text-sm">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-slate-900 truncate max-w-30">
                              {u.nome}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 border-b border-slate-50 text-slate-500 font-mono text-xs">
                          {maskEmail(u.email)}
                        </td>
                        ...
                        <td className="px-6 py-4 border-b border-slate-50">
                          <StatusBadge ativo={u.ativo} bloqueado={!!u.bloqueadoAte} />
                        </td>
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                {filteredUsers.length === 0
                  ? 0
                  : Math.min((page - 1) * PAGE_SIZE + 1, filteredUsers.length)}
                â€“{Math.min(page * PAGE_SIZE, filteredUsers.length)} de {filteredUsers.length}
              </span>
              <div className="flex gap-2">
                ...
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    onClick={() => setPage(p)}
                    className={cn(
                      "w-8 h-8 rounded-full border text-xs font-bold transition-colors",
                      page === p
