...
function getFinancialHealth(receitas: number, gastos: number) {
  if (receitas <= 0) return { label: "Sem dados", color: "text-slate-500", bg: "bg-slate-500", pct: 0 };
...
export default function DashboardPage() {
  ...
  // Category donut
  const categoryTotal = resumo?.gastosPorCategoria?.reduce((s: number, g: { total: number }) => s + g.total, 0) ?? 0;
  ...
  const cardUsedPercent = firstCard && firstCard.limite > 0
      ...
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-white/70 dark:bg-slate-700/70 px-3 py-1.5 rounded-xl border border-white/60 dark:border-slate-600/60 shadow-sm">
            ...
            <button onClick={next} disabled={isCurrentMonth} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-lg transition-colors disabled:opacity-30 cursor-pointer disabled:cursor-not-allowed">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-12 gap-6 xl:gap-8">
          ...
          <div className="col-span-12 xl:col-span-9 space-y-6 xl:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-6">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-36 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
                ...
                <div className="z-10 mt-auto">
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Receitas</p>
                  <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight"><AnimatedCurrency value={resumo.totalReceitas} /></h3>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-36 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
                ...
                <div className="z-10 mt-auto">
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Gastos</p>
                  <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight"><AnimatedCurrency value={resumo.totalGastos} /></h3>
                </div>
              </motion.div>
                ...
                <div className="flex justify-between items-start z-10">
                  ...
                  {resumo.saldo !== 0 && resumo.totalReceitas > 0 && (
                    <span className={cn(
                      ...
                      {resumo.saldo > 0 ? "+" : ""}{Math.round(((resumo.totalReceitas - resumo.totalGastos) / resumo.totalReceitas) * 100)}%
                    </span>
                  )}
                </div>
                <div className="z-10 mt-auto">
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Saldo Atual</p>
                  <h3 className={cn("text-2xl font-bold tracking-tight", resumo.saldo >= 0 ? "text-slate-800 dark:text-white" : "text-red-600 dark:text-red-400")}><AnimatedCurrency value={resumo.saldo} /></h3>
                </div>
              </motion.div>

              {/* % Comprometido */}
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-panel p-5 rounded-2xl flex flex-col justify-between h-36 relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
                ...
                <div className="z-10 mt-auto space-y-2">
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider">% Comprometido</p>
                  <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{comprometimentoReceita !== null ? <AnimatedPercent value={comprometimentoReceita} decimals={0} /> : "—"}</h3>
                  {comprometimentoReceita !== null && (
                    <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5">
                      <div className={cn(
                        "h-1.5 rounded-full transition-all duration-700",
                        comprometimentoReceita <= 50 ? "bg-emerald-500" : comprometimentoReceita <= 80 ? "bg-amber-500" : "bg-red-500"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {!loadingHistorico && historicoData && historicoData.length >= 2 && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2 glass-panel p-6 rounded-2xl">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="font-bold text-slate-800 dark:text-white text-sm">Evolução Semestral</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Receitas vs Gastos</p>
                    </div>
                    <div className="flex items-center gap-4 text-xs font-medium">
                      <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Receitas</span>
                      <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-red-400" /> Gastos</span>
                    </div>
                  </div>
              ...
              {/* Category Donut (1/3) */}
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="glass-panel p-6 rounded-2xl flex flex-col">
                <h3 className="font-bold text-slate-800 dark:text-white text-sm mb-6">Gastos por Categoria</h3>
                {categorySegments.length > 0 ? (
                  <>
                    <div className="flex-1 flex items-center justify-center mb-6">
                      <div className="relative size-40">
                        <div className="size-40 rounded-full shadow-inner" style={{ background: conicGradient }} />
                        <div className="absolute inset-5 bg-white/95 dark:bg-slate-800/95 rounded-full flex flex-col items-center justify-center shadow-inner">
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-semibold tracking-wider">Total</span>
                          <span className="text-lg font-bold text-slate-800 dark:text-white">{formatCurrency(categoryTotal)}</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2.5 mt-auto">
                      {categorySegments.slice(0, 5).map((seg) => (
                        <div key={seg.categoria} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            <span className="size-2.5 rounded-full shrink-0" style={{ backgroundColor: seg.color }} />
                            <span className="text-slate-600 dark:text-slate-400 font-medium truncate max-w-24">{seg.categoria}</span>
                          </div>
                          <span className="font-bold text-slate-800 dark:text-white tabular-nums">{seg.percentual}%</span>
                        </div>
                      ))}
                    </div>
                  </>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Recent Transactions */}
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel p-6 rounded-2xl">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold text-slate-800 dark:text-white text-sm">Últimas Transações</h3>
                  <Link href="/lancamentos" className="text-xs text-emerald-600 font-bold hover:underline flex items-center gap-1">
                ...
                {(lancamentos?.items ?? []).length > 0 ? (
                  <div className="space-y-3">
                    {(lancamentos?.items ?? []).slice(0, 5).map((l) => (
                      <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/50 dark:hover:bg-white/5 transition-colors">
                        <div className={cn(
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">{l.descricao}</p>
                          <p className="text-xs text-slate-400 dark:text-slate-500">{l.categoria} · {formatDate(l.data)}</p>
                        </div>
                        <span className={cn(
                  ...
                  <p className="text-sm text-slate-400 text-center py-8">Nenhuma transação encontrada</p>
                )}
              </motion.div>

              {/* Active Goals */}
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="glass-panel p-6 rounded-2xl">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold text-slate-800 dark:text-white text-sm">Metas Ativas</h3>
                  <Link href="/metas" className="text-xs text-emerald-600 font-bold hover:underline flex items-center gap-1">
                    Ver todas <ArrowRight className="h-3 w-3" />
                  </Link>
                </div>
                {metasAtivas.length > 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <div className={cn(
                              ...
                              meta.percentualConcluido >= 80 ? "bg-emerald-500" : meta.percentualConcluido >= 50 ? "bg-sky-500" : "bg-amber-500"
                            )}>
                              <Target className="h-4 w-4" />
                            </div>
                            <div>
                              <p className="text-sm font-semibold text-slate-800 dark:text-white">{meta.nome}</p>
                  ...
                  <div className="text-center py-8">
                    ...
                    <Link href="/metas" className="text-xs text-emerald-600 font-bold hover:underline mt-2 inline-block">
                      Criar meta
                    </Link>
                  </div>
                )}
          ...
          <aside className="col-span-12 xl:col-span-3 space-y-6">
            {/* Credit Card */}
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="glass-panel p-6 rounded-2xl">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-bold text-slate-800 dark:text-white text-sm">Meus Cartões</h3>
                <Link href="/cartoes" className="text-xs text-emerald-600 font-bold hover:underline">Gerenciar</Link>
              </div>
              {firstCard ? (
                <>
                  {/* Physical card */}
                  <div className="w-full aspect-[1.586] rounded-2xl bg-linear-to-br from-slate-800 to-slate-900 dark:from-slate-700 dark:to-slate-800 p-5 text-white shadow-xl relative overflow-hidden mb-6 group cursor-pointer hover:scale-[1.02] transition-transform duration-300">
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      <span className="text-xs font-bold tracking-wider opacity-80">{firstCard.nome}</span>
                      <CreditCard className="h-5 w-5 opacity-60" />
                    </div>
                    <div className="absolute bottom-5 left-5 right-5 z-10">
                      <p className="text-sm font-mono tracking-[.2em] mb-3 opacity-90">•••• •••• •••• ••••</p>
                      <div className="flex justify-between items-end text-[10px] opacity-70">
                        <span className="uppercase tracking-wider">{usuario?.nome ?? "TITULAR"}</span>
                  ...
                  <div>
                    ...
                    <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2">
                      <div className={cn(
                        "h-2 rounded-full transition-all duration-700",
                        cardUsedPercent <= 50 ? "bg-emerald-500" : cardUsedPercent <= 80 ? "bg-amber-500" : "bg-red-500"
                      )} style={{ width: `${Math.min(cardUsedPercent, 100)}%` }} />
                    </div>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5">{cardUsedPercent}% utilizado</p>
                  </div>
                </>
              ) : (
                <div className="text-center py-6">
                  ...
                  <Link href="/cartoes" className="text-xs text-emerald-600 font-bold hover:underline mt-2 inline-block">
                    Adicionar cartão
                  </Link>
                </div>
              )}
            ...
            {/* Telegram */}
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="glass-panel p-6 rounded-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center mt-2">
                ...
                <h3 className="font-bold text-slate-800 dark:text-white mb-2 text-sm">Telegram Alertas</h3>
            ...
            {/* Tip of the Day */}
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="glass-panel p-6 rounded-2xl">
              <h3 className="font-bold mb-3 text-[11px] uppercase tracking-wider text-slate-400 dark:text-slate-500">Dica do Dia</h3>
              <div className="flex gap-4">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 italic leading-relaxed font-medium">
                  &ldquo;Pagar à vista com desconto é sempre melhor do que investir e parcelar, a menos que o desconto seja muito baixo.&rdquo;
                </p>
              </div>
            </motion.div>
          </aside>
